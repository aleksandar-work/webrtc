...
<footer class="sticky-footer bg-white">
    <div class="container my-auto">
        <div class="copyright text-center my-auto">
            <span>Copyright &copy; DanDash <?php echo date('Y'); ?></span>
        </div>
    </div>
</footer>
...
<?php if ($basename == 'history.php') { ?>


    <script>


        var agentId = <?php echo json_encode($_GET['agentId']) ?>;
        console.log(agentId);
        ...
        jQuery(document).ready(function ($) {
            ...
            $.ajax({
                type: 'POST',
                url: '../server/script.php',
                data: {'type': 'getrecordings','agentId': agentId}
            })
                .done(function (data) {
                    if (data) {
                        var result = JSON.parse(data);
...
console.log(result);
                        ...
                        $.each(result, function (i, item) {
                            ...
                            html += '<div class="col-xl-3 col-md-6 mb-4">' +
                                ...
                                '<div class="col mr-2">' +
                                '<div class="text-xs font-weight-bold text-success text-uppercase mb-1">Doctor: '+item.roomId+'</div>' +
                                '</div>' +
                                '<div class="col-auto">' +
                                ...
                                '<div class="row no-gutters align-items-center">' +
                                '<div class="col mr-1">' +
                                '<div class="text-xl-center font-weight-bold text-dark text-uppercase mb-1">'+item.agent+'</a></div>' +
                                '</div>' +
                                '</div>' +
                                '<div style="float: right"><a target="_blank" class="btn btn-info" title="Conference visitor URL" href="' + item.agenturl + '">Enter Room</a></div>' +
                                '</div>' +
                                '</div>' +
                                '</div>';

                        });
                        $('#room_entry').html(html);
                    ...
                    if (data) {
                        ...
                        $.each(result, function (i, item) {
                            html += ' <div class="col-xl-3 col-md-6 mb-4">\n' +
                                ...
                                '                <div class="row no-gutters align-items-center">\n' +
                                '                    <div class="col mr-2">\n' +
                                '                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1"><a target="_blank" class="text-xs font-weight-bold text-info text-uppercase mb-1" href="history.php?agentId=' + item.agent_id + '">Doctor :' + i + '</a></div>\n' +
                                '                        <div class="col mr-2">\n' +
                                '                            <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Total Conversations: ' + item.conversations + '</div>\n' +
                                '                        </div>\n' +
                                '                    </div>\n' +
                                '                    <div class="col-auto">\n' +
                                ...
                                '                    <div class="col mr-2">\n' +
                                '                        <div class="col mr-2">\n' +
                                '                            <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Total Recordings: ' + item.recordings.recording_num + '</div>\n' +
                                '                        </div>\n' +
                                '                    </div>\n' +
                                '                </div>\n' +
                                '            </div>\n' +
